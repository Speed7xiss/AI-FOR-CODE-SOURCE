...
> ⚠️ **A Runa AI não tem acesso ao seu computador.** Não existe agente local,
...
```bash
# instale um ou mais modelos de texto; todos os modelos instalados aparecem no seletor da interface
...
Para usar um endpoint cloud compatível com a API do Ollama, configure também:

```env
OLLAMA_BASE_URL=https://ollama.com
OLLAMA_API_KEY=sua-chave-local
OLLAMA_MODEL=seu-modelo-cloud
```

`OLLAMA_API_KEY` fica somente no backend e nunca é enviado ao navegador.

`OLLAMA_MODEL` é o modelo padrão. Depois que o Ollama estiver em execução, a
Runa consulta os modelos instalados e permite escolher outro no seletor
**Modelo** do cabeçalho. Para adicionar opções, execute `ollama pull
<nome-do-modelo>`; não é necessário editar o frontend. A rota `GET /api/models`
também expõe essa lista para integrações externas.
...
2. Escolha o modelo no seletor **Modelo**, digite uma mensagem ou anexe arquivos (📎 ou arraste e solte).
...
- **Frontend**: hospede a pasta `frontend/` em Cloudflare Pages, Vercel ou
  GitHub Pages (é estático). Configure `window.RUNA_API_BASE_URL` no
